...
| `onWin`| float | no | 0 | The percentage increase to add to the next `betAmount` if round ends up in a **win** |
| `onLoss`| float | no | 0 | The percentage increase to add to the next `betAmount` if round ends up in a **loss**|
...
| `autobetPayout` | This  tallies up the total payout given to player for all the bets placed|
...
:::info `originalBetAmount` is needed to be saved in memory in case the `betAmount` changes due to onWin and onLoss &needed to be reset to originl bet
:::
